...
const ALL_TEAMS = ["All", "≤18 Players", "19-21 Players", "22 Players"];

function playerBucket(n: number) {
  if (n <= 18) return "≤18 Players";
  if (n <= 21) return "19-21 Players";
  return "22 Players";
}
...
export default function GalleryClient() {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/matches")
      .then(r => r.json())
      .then((data: MatchMeta[]) => { setMatches(data); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  const filtered = matches
    .filter(m => {
      if (teamFilter !== "All" && playerBucket(m.players) !== teamFilter) return false;
      ...
      <div className="wrap" style={{ paddingTop: "2rem", paddingBottom: "3rem" }}>
        {loading ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: "1.25rem" }}>
            {[1,2,3].map(i => (
              <div key={i} style={{ height: "320px", background: "#111", border: "1px solid #1a1a1a", borderRadius: "1rem", opacity: 0.5 }} />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "5rem 0", color: "#555" }}>
            <div style={{ fontSize: "3rem", marginBottom: "1rem" }}>⚽</div>
            <p style={{ fontSize: "1rem", fontWeight: 600, marginBottom: "0.4rem", color: "#888" }}>
              {matches.length === 0 ? "No matches processed yet" : "No matches match your filters"}
            </p>
            <p style={{ fontSize: "0.85rem" }}>
              {matches.length === 0 ? "Run the pipeline on a video to see it here." : "Try adjusting your search or filters."}
            </p>
          </div>
        ) : (
          <>
                  ...
                  <div style={{ padding: "1.25rem" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.5rem", marginBottom: "1rem" }}>
                      {[
                        ...
                        { icon: Film,  label: "Frames",   value: `${(match.frames / 1000).toFixed(1)}k` },
                    ...
                    <button
                      onClick={() => router.push(`/gallery/${match.id}`)}
                      style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", padding: "0.6rem", borderRadius: "0.75rem", background: "rgba(212,175,55,0.1)", border: "1px solid rgba(212,175,55,0.35)", color: "#D4AF37", fontSize: "0.85rem", fontWeight: 700, cursor: "pointer", transition: "all 0.2s" }}
